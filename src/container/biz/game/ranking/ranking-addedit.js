...
import {
    isUndefined, getQueryString, showSucMsg, getUserId, getRules,
    getRealValue
} from 'common/js/util';
...
class RankingAddEdit extends React.Component {
    ...
    componentDidMount() {
        Promise.all([
            getQiniuToken(),
            getDictList({parentKey: 'match'}),
            getDictList({parentKey: 'player_status'}),
            fetch(640016, {code: this.playerCode}),
            fetch(640026, {code: this.code})
        ]).then(([token, match, playerStatus, player, rank]) => {
            this.setState({
                player,
                rank,
                selectData: {status: playerStatus, match},
